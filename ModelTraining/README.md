...
60-second audio samples.  Derives the test set from `output/csv/detections.csv`
(generated by `make_csv.py`) by excluding rows whose URI appears in
`output/csv/training_samples.csv` (generated by `extract_training_samples.py`),
then runs each enabled model on the corresponding WAV files under `output/testing-wav/`
...
| `--podsai-model-path` | Path or Hub ID for PODS-AI model. Required when `podsai` is in `--models` |
...
Evaluating model: fastai
  [fastai] resident/rpi_orcasound_lab/2023_08_18_00_59_53_PST: predicted='resident' → correct
  ...
