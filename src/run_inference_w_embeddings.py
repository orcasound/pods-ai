...
def _as_tensor(value: Any) -> Any:
    """Return a torch Tensor-like object from common HuggingFace output shapes."""
...
def _capture_ast_cls_embeddings(model: Any) -> tuple[list[list[float]], Any]:
    """Attach a hook that captures one AST CLS embedding per inference window."""
    embeddings: list[list[float]] = []
    ast_module = getattr(getattr(model, "model", None), "audio_spectrogram_transformer", None)
    if ast_module is None or not hasattr(ast_module, "register_forward_hook"):
        raise ValueError(
            "AST embedding export is only available for PODS-AI Audio Spectrogram "
            "Transformer models."
        )
    ...
    def hook(_module: Any, _inputs: Any, output: Any) -> None:
        ...
        if tensor is None or not hasattr(tensor, "detach"):
            return
        # AST sequence output is [batch, tokens, hidden]. Token 0 is the CLS embedding
        # used by the classifier head.
    ...
    return embeddings, ast_module.register_forward_hook(hook)


def save_ast_embeddings_csv(
    csv_path: str,
    embeddings: list[list[float]],
    wav_path: str,
    hop_duration: float,
    ...
    """Save per-window AST CLS embeddings to CSV."""
    output_path = Path(csv_path)
    if output_path.parent:
        output_path.parent.mkdir(parents=True, exist_ok=True)

    max_embedding_width = max((len(row) for row in embeddings), default=0)
    fieldnames = [
        "wav_path",
        "segment_index",
        "start_time_seconds",
        "duration_seconds",
        *[f"embedding_{idx}" for idx in range(max_embedding_width)],
    ]
    ...
    with output_path.open("w", newline="", encoding="utf-8") as csv_file:
        ...
        for segment_index, embedding in enumerate(embeddings):
            row: dict[str, Any] = {
                ...
                "start_time_seconds": segment_index * float(hop_duration),
                "duration_seconds": float(segment_duration),
            }
            row.update(
                {
...
def run_inference(wav_path: str, model_type: str = "podsai",
                  ...
                  start_time_utc: Optional[datetime] = None,
                  ast_embeddings_csv: Optional[str] = None,
                  return_ast_embeddings: bool = False) -> dict:
    """
    ...
    Args:
        ...
        ast_embeddings_csv: Optional CSV output path for per-window AST CLS embeddings.
                            Only supported for PODS-AI AST models.
        return_ast_embeddings: If True, include per-window AST CLS embeddings in
                               the returned dictionary. Only supported for PODS-AI
                               AST models.
    ...
    if model_type == "fastai":
        ...
        ast_embeddings: list[list[float]] = []
        hook_handle = None
        if ast_embeddings_csv is not None or return_ast_embeddings:
            ast_embeddings, hook_handle = _capture_ast_cls_embeddings(model)
        ...
        if ast_embeddings_csv is not None:
            save_ast_embeddings_csv(
                ast_embeddings_csv,
                ast_embeddings,
                wav_path=wav_path,
            ...
            print(f"Saved AST embeddings CSV: {ast_embeddings_csv}")
    ...
    if return_ast_embeddings:
        output["ast_embeddings"] = ast_embeddings
    return output
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional CSV output path for per-window AST CLS embeddings. "
            "Only valid with --model podsai and the AST model variant."
        ),
    )
    ...
    with ExitStack() as stack:
        ...
        try:
            results = run_inference(
                ...
                start_time_utc=start_time_utc,
                ast_embeddings_csv=args.ast_embeddings_csv,
            )
        except ValueError as e:
