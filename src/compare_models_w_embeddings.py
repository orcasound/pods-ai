...
"""
Compare multiple models on a test set of audio samples and export PODS-AI embeddings.

Usage:
    python compare_models_w_embeddings.py [options]
...
from run_inference_w_embeddings import run_inference
...
def write_podsai_embedding_rows(
    embeddings_csv: Path,
    sample: TestSample,
    wav_path: Path,
    inference_result: dict,
    model_type: str,
    window_confusion_matrix: dict[str, dict[str, int]],
) -> int:
    """Append one PODS-AI AST embedding row per local prediction window."""
    embeddings = inference_result.get("ast_embeddings", [])
    if not embeddings:
        return 0
    ...
    fieldnames = [
        *EMBEDDING_CSV_BASE_FIELDS,
        *[f"embedding_{idx}" for idx in range(embedding_width)],
    ]
    ...
    with embeddings_csv.open("a", newline="", encoding="utf-8") as csv_file:
        ...
        for segment_index, embedding in enumerate(embeddings):
            ...
            actual_label = ground_truth_label
            if actual_label not in window_confusion_matrix:
                window_confusion_matrix[actual_label] = {}
            predicted_counts = window_confusion_matrix[actual_label]
...
def evaluate_model(
    ...
    podsai_embeddings_csv: Optional[Path] = None,
    window_confusion_matrix: Optional[dict[str, dict[str, int]]] = None,
) -> ModelResult:
    """
    ...
    Args:
        ...
        podsai_embeddings_csv: Optional CSV path for per-window AST embeddings.
                              Only used when result_model_type is "podsai".
        window_confusion_matrix: Optional window-level confusion matrix updated
                                 from the exported embedding rows.
    ...
    for sample in samples:
        ...
        try:
            export_embeddings = (
                podsai_embeddings_csv is not None
                and (result_model_type or model_type) == "podsai"
            )
            inference_result = run_inference(str(wav_path), model_type=model_type,
                                             model_path=model_path,
                                             model_revision=model_revision,
                                             return_ast_embeddings=export_embeddings)
            predict_time = inference_result.get("predict_time", 0.0)
            result.predict_times.append(predict_time)
            if export_embeddings:
                num_embedding_rows = write_podsai_embedding_rows(
                    podsai_embeddings_csv,
                    sample=sample,
                    wav_path=wav_path,
                    inference_result=inference_result,
                    model_type=result_model_type or model_type,
                    window_confusion_matrix=window_confusion_matrix
                    if window_confusion_matrix is not None
                    else {},
                )
                print(f"    saved {num_embedding_rows} PODS-AI embedding rows")
...
def main() -> int:
    ...
    parser.add_argument(
        "--podsai-embeddings-csv",
        default="output/csv/podsai_window_embeddings.csv",
        help=(
            "CSV path for per-window PODS-AI AST embeddings "
            "(default: output/csv/podsai_window_embeddings.csv)."
        ),
    )
    ...
    podsai_embeddings_csv = Path(args.podsai_embeddings_csv)
    if "podsai" in models and podsai_embeddings_csv.exists():
        podsai_embeddings_csv.unlink()
    if "podsai" in models:
        print(f"PODS-AI embeddings CSV: {podsai_embeddings_csv}")
    print()

    results = []
    podsai_window_confusion_matrix: dict[str, dict[str, int]] = {}
    for model_type in models:
        ...
        model_result = evaluate_model(
            ...
            podsai_embeddings_csv=podsai_embeddings_csv,
            window_confusion_matrix=podsai_window_confusion_matrix,
        )
        results.append(model_result)
        print()

    print_summary(results)
    if podsai_window_confusion_matrix:
        print()
        print_confusion_matrix(
            ModelResult(
                model_type="podsai window embeddings",
                confusion_matrix=podsai_window_confusion_matrix,
            )
        )
        print(f"PODS-AI window embeddings written to {podsai_embeddings_csv}")
    return 0
